...
public class ExtraLeftMenuDarkAdapter extends BaseAdapter{
    private Context context;
    private ArrayList<ExtraLeftMenuDarkModel> menuData;
    private LayoutInflater mInflater;

    public ExtraLeftMenuDarkAdapter(Context ctx, ArrayList<ExtraLeftMenuDarkModel> data){
        context = ctx;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.row_extra_leftmenu_dark, null);
        ...
        if(menuData.get(position).isSelected()) {
            ...
            holder.menuText.setTextColor(ContextCompat.getColor(context, R.color.extraLeftMenu3menuFontActive));
            holder.menuText.setTypeface(null, Typeface.BOLD);
            holder.buttonMenu.setBackgroundColor(ContextCompat.getColor(context, R.color.extraLeftMenu3menuActiveBg));

        }else{
            holder.barNotificationActive.setVisibility(View.INVISIBLE);
            holder.menuText.setText(menuData.get(position).getMenuName());
            holder.menuText.setTextColor(ContextCompat.getColor(context, R.color.extraLeftMenu3menuFont));
            holder.menuText.setTypeface(null, Typeface.NORMAL);
            holder.buttonMenu.setBackgroundColor(ContextCompat.getColor(context, R.color.extraLeftMenu3menuBg));
        }
